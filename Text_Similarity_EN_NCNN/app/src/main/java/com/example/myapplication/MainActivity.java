...
public class MainActivity extends AppCompatActivity {
    ...
    private static final float threshold_similar = 0.1f;  // You can print the max_score to assign a appropriate value.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Load_Models_0(mgr,false,false,false)) { // GPU failed for GTE model, It seems to model FP16 problems.
            ...
            for (int i = 0; i < amount_of_commands; i++) {
                score_data_commands[i] = Run_Text_Embedding(input_token_GTE, Tokenizer(commands.get(i)), model_hidden_size_GTE);
    ...
    private static int[] Compare_Similarity(int count) {
        float[] model_result = Run_Text_Embedding(input_token_GTE, count, model_hidden_size_GTE);
    ...
    private native boolean Load_Models_0(AssetManager assetManager, boolean USE_INT8, boolean USE_FP16, boolean USE_GPU);
    private static native float[] Run_Text_Embedding(int[] token_index, int words_count, int model_hidden_size);
}
