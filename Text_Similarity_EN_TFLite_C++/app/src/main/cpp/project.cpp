...
bool report_error_armnn_delegate(const char* errorMessage) {  // You can edit it by yourself.
    return true;
}
...
Java_com_example_myapplication_MainActivity_Load_1Models_10(JNIEnv *env, jobject clazz,
    ...
    std::unique_ptr<tflite::FlatBufferModel> model_file = tflite::FlatBufferModel::BuildFromBuffer(flatBuffer_A.data(),fileSize);
    if (model_file == nullptr) {
        return JNI_FALSE;
    }
    tflite::ops::builtin::BuiltinOpResolver resolver;
    tflite::InterpreterBuilder(*model_file,resolver)(&interpreter_A);
