...
Java_com_example_myapplication_MainActivity_Load_1Models_10(JNIEnv *env, jclass clazz,
    ...
    config.set_threads(4);  // If the inference takes a very long time, the issue may not lie with the demo code. There seems to be a bug in the interaction between your device and the Paddle-Lite framework. Maybe try another device.
