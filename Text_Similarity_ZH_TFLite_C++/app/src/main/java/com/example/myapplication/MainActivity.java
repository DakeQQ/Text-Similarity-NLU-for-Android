...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int max_token_limit_GTE = 25;  // Replace with your max input size.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Load_Models_0(mgr,false,false,true,false,false,false,false,false)) {
            // (ArmNN | GPU | NNAPI | DSP) can not use at the same time.
            // ArmNN usually failed while using the multi-converter "Pytorch->ONNX->TFLite". It seems to the ArmNN model analyzer problems.
            // The GTE model time cost: fp16 <= xnnpack ~ nnapi ~ (fp16+nnapi) <= (gpu+fp16) ~ (gpu+fp16+xnnpack) <= gpu. Failed: (fp16+xnnpack), ArmNN. No test device: DSP
            ...
            for (int i = 0; i < amount_of_commands; i++) {
                score_data_commands[i] = Run_Text_Embedding(input_token_GTE, Tokenizer(commands.get(i)), model_hidden_size_GTE, max_token_limit_GTE);
    ...
    private static int[] Compare_Similarity(int count) {
        float[] model_result = Run_Text_Embedding(input_token_GTE, count, model_hidden_size_GTE, max_token_limit_GTE);
    ...
    private native boolean Load_Models_0(AssetManager assetManager,boolean USE_UINT8, boolean USE_INT8, boolean USE_FP16, boolean USE_XNNPACK, boolean USE_ARM, boolean USE_GPU, boolean USE_NNAPI, boolean DSP_NPU);
    private static native float[] Run_Text_Embedding(int[] token_index, int words_count, int model_hidden_size, int max_token_limit);
}
