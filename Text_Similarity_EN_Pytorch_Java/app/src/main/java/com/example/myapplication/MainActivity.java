...
import static org.pytorch.MemoryFormat.CONTIGUOUS;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static Module model_A;

    // Due to the temporary problems in 'org.pytorch:pytorch_android_lite:2.1.0', These loadLibrary are necessary.
    // If you use 'org.pytorch:pytorch_android_lite:1.13.1', these loadLibrary can be deleted, which could lower memory usage but increase the time cost.
    static {
    ...
    private boolean Load_Models_0(AssetManager mgr, boolean USE_GPU) {
        try {
            PyTorchAndroid.setNumThreads(cpu_threads);
            if (USE_GPU) {
                model_A = LiteModuleLoader.loadModuleFromAsset(mgr,"Model_GTE.ptl", Device.VULKAN);
            } else {
                model_A = LiteModuleLoader.loadModuleFromAsset(mgr,"Model_GTE.ptl", Device.CPU);
            }
            return true;
        } catch (Exception e) {
            return false;
    ...
    private static float[] Run_Text_Embedding(int words_count) {
        float[] attention_mask = new float[max_token_limit_GTE];
        Arrays.fill(attention_mask,0, words_count,1.f);
        Arrays.fill(attention_mask, words_count, max_token_limit_GTE,-9999999999.f);
        return model_A.forward(IValue.from(Tensor.fromBlob(input_token_GTE, new long[]{1, max_token_limit_GTE}, CONTIGUOUS)), IValue.from(Tensor.fromBlob(attention_mask, new long[]{1, max_token_limit_GTE}, CONTIGUOUS))).toTensor().getDataAsFloatArray();
    }
}
